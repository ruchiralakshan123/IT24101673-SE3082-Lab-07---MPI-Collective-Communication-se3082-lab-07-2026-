...
int main(int argc, char **argv) {
    ...
    /*
     * ALLREDUCE: Every process gets the global total_sum.
     * No root parameter — result is delivered to ALL processes.
     * More efficient than MPI_Reduce + MPI_Bcast.
     */
    long long total_sum = 0;
    MPI_Allreduce(&local_sum, &total_sum, 1, MPI_LONG_LONG, MPI_SUM, MPI_COMM_WORLD);

    /* Every process prints its contribution */
